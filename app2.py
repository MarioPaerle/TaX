"""
SSH Manager v2 — EXPERIMENTAL
Adds a Groq AI assistant panel to the file editor.
Extra dependency: pip install groq

Set your Groq API key either:
  - In the UI on the Files page (stored in session only)
  - Or as env var: GROQ_API_KEY=your_key_here
"""

# ── Auto-installer ────────────────────────────────────────────────────────────
def _bootstrap():
    import importlib.util, subprocess, sys, os, urllib.request, tempfile
    REQUIRED = ['flask', 'flask_sock', 'paramiko', 'groq']
    missing  = [pkg for pkg in REQUIRED if importlib.util.find_spec(pkg) is None]
    if not missing: return
    print(f'\n[SSH Manager v2] Installing: {", ".join(missing)}')
    try:
        subprocess.check_call([sys.executable, '-m', 'pip', 'install', '--user',
                                'flask', 'flask-sock', 'paramiko', 'groq'],
                              stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        print('[SSH Manager v2] Install complete.\n')
        return
    except Exception: pass
    print('[SSH Manager v2] pip not found, bootstrapping...')
    try:
        tmp = tempfile.mktemp(suffix='.py')
        urllib.request.urlretrieve('https://bootstrap.pypa.io/get-pip.py', tmp)
        subprocess.check_call([sys.executable, tmp, '--user', '--quiet'])
        import os; os.remove(tmp)
        subprocess.check_call([sys.executable, '-m', 'pip', 'install', '--user',
                                'flask', 'flask-sock', 'paramiko', 'groq'])
        print('[SSH Manager v2] Install complete.\n')
    except Exception as e:
        print(f'[SSH Manager v2] ERROR: {e}'); import sys; sys.exit(1)

_bootstrap()
# ─────────────────────────────────────────────────────────────────────────────

import os, select, threading, time, uuid, stat as stat_module, base64, mimetypes
from flask import Flask, request, session, redirect, url_for, jsonify, render_template_string, Response, stream_with_context
from flask_sock import Sock
import paramiko

app = Flask(__name__)
app.secret_key = 'ssh-manager-v2-secret-2024'
sock = Sock(app)
...
.btn-ai{background:var(--purple);color:#080c14;border-color:var(--purple);font-weight:600}
...
.logo-ai{font-size:9px;color:var(--purple);margin-top:2px;letter-spacing:.05em}
...
<aside class="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="/files" class="nav-item {{ 'active' if active=='files' else '' }}">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M3 7a2 2 0 012-2h5l2 2h7a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V7z"/></svg>Files + AI
    </a>
...
body{display:flex;align-items:center;justify-content:center;height:100vh;
  background-image:radial-gradient(ellipse at 20% 60%,rgba(56,189,248,.05) 0%,transparent 55%),
                   radial-gradient(ellipse at 80% 40%,rgba(192,132,252,.05) 0%,transparent 55%)}
.card{width:420px;background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:36px 32px 32px}
...
.logo-ai{font-size:10px;color:var(--purple);margin-top:3px;letter-spacing:.05em}
...
<div class="card">
  <div class="logo">⬡ SSH MANAGER v2</div>
  <div class="logo-sub">cluster file manager &amp; terminal</div>
  <div class="logo-ai">✦ experimental — Groq AI assistant inside the editor</div>
  <div class="card-body">
...
/* File tree */
.filetree{width:230px;flex-shrink:0;background:var(--surface);border-right:1px solid var(--border);display:flex;flex-direction:column}
.tree-header{display:flex;align-items:center;gap:3px;padding:6px 8px;border-bottom:1px solid var(--border);flex-shrink:0;flex-wrap:wrap}
.path-bar{width:100%;font-size:10px;color:var(--muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;padding:0 2px 4px}
...
.status-dot.dirty{color:var(--yellow)}.status-dot.clean{color:var(--green)}
...
.ai-panel.collapsed{width:32px}
.ai-toggle{display:flex;align-items:center;justify-content:center;padding:10px 0;border-bottom:1px solid var(--border);
  cursor:pointer;color:var(--purple);font-size:11px;letter-spacing:.06em;flex-shrink:0;gap:6px;
  transition:background .1s}
.ai-toggle:hover{background:var(--surface2)}
.ai-panel.collapsed .ai-body{display:none}
.ai-body{display:flex;flex-direction:column;flex:1;overflow:hidden}
.ai-key-row{display:flex;gap:6px;padding:8px;border-bottom:1px solid var(--border);flex-shrink:0}
.ai-key-row input{font-size:11px}
.ai-model-row{padding:6px 8px;border-bottom:1px solid var(--border);flex-shrink:0}
.ai-model-row select{font-size:11px}
.ai-messages{flex:1;overflow-y:auto;padding:8px;display:flex;flex-direction:column;gap:8px}
.ai-msg{font-size:11px;line-height:1.6;padding:8px 10px;border-radius:5px;white-space:pre-wrap;word-break:break-word}
.ai-msg.user{background:var(--accent-dim);color:var(--text);align-self:flex-end;max-width:90%;border:1px solid rgba(56,189,248,.2)}
.ai-msg.assistant{background:var(--surface2);color:var(--text);align-self:flex-start;max-width:95%;border:1px solid var(--border)}
.ai-msg.system{color:var(--muted);font-size:10px;text-align:center;background:none}
.ai-input-area{padding:8px;border-top:1px solid var(--border);display:flex;flex-direction:column;gap:6px;flex-shrink:0}
.ai-actions{display:flex;gap:5px;flex-wrap:wrap}
.ai-textarea{resize:none;height:70px;font-size:11px;line-height:1.5}
.ai-send-row{display:flex;gap:6px}
.ai-send-row button{flex:1}
.typing-dot{display:inline-block;animation:tdot 1.2s ease-in-out infinite}
...
<div class="page">
  ...
  <div class="filetree">
    <div class="tree-header">
      ...
      <button class="icon-btn del-btn" onclick="deleteSelected()" title="Delete selected">✕</button>
    </div>
    <div class="file-list" id="fileList">
      <div class="file-item"><span style="color:var(--muted);padding:0 12px;font-size:12px">Loading…</span></div>
    </div>
  </div>
  ...
  <div class="ai-panel" id="aiPanel">
    <div class="ai-toggle" onclick="toggleAI()" title="Toggle AI panel">
      ...
      <span id="aiToggleLabel">GROQ AI</span>
    </div>
    <div class="ai-body">
      <div class="ai-key-row">
        <input type="password" id="groqKey" placeholder="Groq API key  gsk_…" value="{{ groq_key }}">
        <button class="btn btn-sm" onclick="saveKey()">Set</button>
      </div>
      ...
      <div class="ai-input-area">
        <div class="ai-actions">
          <button class="btn btn-sm" onclick="aiAction('explain')">Explain</button>
          <button class="btn btn-sm" onclick="aiAction('fix')">Fix errors</button>
          <button class="btn btn-sm" onclick="aiAction('optimize')">Optimize</button>
          <button class="btn btn-sm" onclick="aiAction('docstring')">Docstrings</button>
          <button class="btn btn-sm" onclick="aiAction('selection')">Selected code</button>
        </div>
        <textarea class="ai-textarea" id="aiInput" placeholder="Ask something about the open file, or type a question…"></textarea>
        <div class="ai-send-row">
          <button class="btn btn-ai btn-sm" onclick="sendAI()">Ask ✦</button>
          <button class="btn btn-sm btn-danger" onclick="clearChat()">Clear</button>
        </div>
      </div>
    </div>
  </div>
...
function mkItem(name,isDir,path){
  ...
  div.addEventListener('click',()=>{
    ...
    div.classList.add('active');selectedPath=path;
    if(isDir)loadDir(path);
    else if(isImage(name))openImage(path,name);
    else openFile(path,name);
  });
  div.addEventListener('contextmenu',e=>{if(name==='..')return;e.preventDefault();showCtx(e.clientX,e.clientY,path,isDir,name)});
  return div;
}
...
async function deleteSelected(){
  if(!selectedPath)return alert('Select a file first');
...
function toggleAI(){
  ...
  p.classList.toggle('collapsed',aiCollapsed);
  document.getElementById('aiToggleLabel').style.display=aiCollapsed?'none':'';
}

function saveKey(){
  const key=document.getElementById('groqKey').value.trim();
  fetch('/api/ai/set_key',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({key})});
}
...
function clearChat(){
  chatHistory=[];
  document.getElementById('aiMessages').innerHTML='<div class="ai-msg system">✦ Chat cleared.</div>';
}

function aiAction(action){
  if(!openPath&&action!=='selection'){addAIMsg('system','⚠ Open a file first.');return}
  const prompts={
    explain:'Explain what this code does, clearly and concisely.',
    fix:'Find and fix any bugs or errors in this code. Show the corrected version.',
    optimize:'Suggest optimizations for performance and readability.',
    docstring:'Add proper docstrings and comments to this code.',
    selection:'Explain or improve the selected code snippet.',
  };
...
async function sendAI(){
  ...
  const key=document.getElementById('groqKey').value.trim();
  if(!key){addAIMsg('system','⚠ Set your Groq API key first.');return}
  ...
  try{
    ...
    const d=await r.json();
    if(d.error){typingDiv.className='ai-msg system';typingDiv.textContent='⚠ '+d.error;return}
    typingDiv.textContent=d.reply;
    chatHistory.push({role:'assistant',content:d.reply});
    document.getElementById('aiMessages').scrollTop=document.getElementById('aiMessages').scrollHeight;
  }catch(e){
    typingDiv.className='ai-msg system';
    typingDiv.textContent='⚠ Network error: '+e.message;
  }
}
</script>
"""
...
<div class="page">
  ...
  <div class="content">
    ...
    <div class="card">
      ...
      <div class="card-body">
        <div class="input-row"><input id="cmdInput" placeholder="sinfo, sacct -j 12345 …"><button class="btn btn-sm" onclick="runCmd()">Run →</button></div>
...
async function loadQueue(){const btn=document.getElementById('refreshBtn');btn.innerHTML='<span class="spinning">↻</span>';const r=await fetch('/api/slurm/queue');const d=await r.json();btn.innerHTML='↻ Refresh';const body=document.getElementById('queueBody');if(d.error){body.innerHTML=`<tr><td class="empty-row" colspan="9" style="color:var(--red)">${d.error}</td></tr>`;return}document.getElementById('badge').textContent=d.jobs.length?`${d.jobs.length} jobs`:'idle';if(!d.jobs.length){body.innerHTML='<tr><td class="empty-row" colspan="9">No jobs in queue</td></tr>';return}body.innerHTML=d.jobs.map(j=>`<tr><td style="color:var(--accent);font-weight:500">${j.id}</td><td>${j.name}</td><td><span class="tag ${stateClass(j.state)}">${j.state}</span></td><td>${j.time}</td><td>${j.time_limit}</td><td>${j.nodes}</td><td>${j.partition}</td><td style="color:var(--muted);font-size:10px">${j.reason}</td><td><button class="btn btn-danger btn-sm" onclick="cancelJob('${j.id}')">✕</button></td></tr>`).join('')}
async function cancelJob(id){if(!confirm(`Cancel ${id}?`))return;const r=await fetch('/api/slurm/cancel',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({job_id:id})});const d=await r.json();if(d.error)alert(d.error);else loadQueue()}
async function submitJob(){const path=document.getElementById('scriptPath').value.trim();if(!path)return;const r=await fetch('/api/slurm/submit',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({script_path:path})});const d=await r.json();const log=document.getElementById('submitLog');log.style.display='';const isErr=!!d.error&&!d.output;log.className='log'+(isErr?' err':'');log.textContent=d.output||d.error||'Done.';if(!isErr)setTimeout(loadQueue,800)}
async function runCmd(){const cmd=document.getElementById('cmdInput').value.trim();if(!cmd)return;const r=await fetch('/api/run',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({command:cmd})});const d=await r.json();const log=document.getElementById('cmdLog');log.style.display='';log.className='log'+(!d.stdout&&(d.stderr||d.error)?' err':'');log.textContent=(d.stdout||'')+(d.stderr||'')||d.error||'(no output)'}
...
const term=new Terminal({fontFamily:"'Fira Code','Courier New',monospace",fontSize:13,lineHeight:1.5,cursorBlink:true,scrollback:8000,
  theme:{background:'#050911',foreground:'#cdd9e5',cursor:'#38bdf8',cursorAccent:'#050911',selectionBackground:'rgba(56,189,248,0.2)',
    black:'#1e2d45',brightBlack:'#546070',red:'#f87171',brightRed:'#fca5a5',green:'#4ade80',brightGreen:'#86efac',
    yellow:'#fbbf24',brightYellow:'#fde68a',blue:'#38bdf8',brightBlue:'#7dd3fc',magenta:'#a78bfa',brightMagenta:'#c4b5fd',
    cyan:'#22d3ee',brightCyan:'#67e8f9',white:'#cdd9e5',brightWhite:'#f4f4f5'}});
const fitAddon=new FitAddon.FitAddon();
term.loadAddon(fitAddon);term.open(document.getElementById('termWrap'));fitAddon.fit();
...
def files():
    ...
    return render_template_string(FILES_HTML, active='files', host=d['host'],
                                  username=d['username'], home=d['home'],
...
@app.route('/api/files/read_b64')
def api_read_b64():
    d = get_sess()
    if not d: return jsonify({'error': 'Not connected'}), 401
    path = request.args.get('path')
    try:
        with d['sftp'].open(path, 'rb') as f: data = f.read()
        mime = mimetypes.guess_type(path)[0] or 'application/octet-stream'
        return jsonify({'data': base64.b64encode(data).decode(), 'mime': mime})
    except Exception as e: return jsonify({'error': str(e)}), 500
...
def api_ai_set_key():
    d = get_sess()
    if not d: return jsonify({'error': 'Not connected'}), 401
    d['groq_key'] = request.json.get('key', '').strip()
    return jsonify({'success': True})
...
def api_ai_chat():
    ...
    if not d: return jsonify({'error': 'Not connected'}), 401
    body = request.json
    key = body.get('key') or d.get('groq_key', '') or os.environ.get('GROQ_API_KEY', '')
    if not key: return jsonify({'error': 'No Groq API key set. Paste it in the AI panel.'}), 400

    try:
        from groq import Groq
        client = Groq(api_key=key)
        system = ("You are an expert programming assistant embedded in an SSH cluster manager. "
                  "Help the user understand, fix, and improve their code. "
                  "Be concise and practical. When showing code, use markdown code blocks.")
...
if __name__ == '__main__':
    ...
    print('\n  ⬡ SSH Manager v2 (Groq AI) → http://localhost:5000\n')
